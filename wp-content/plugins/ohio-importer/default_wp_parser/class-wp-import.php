 ...
 */
class Ohio_WP_Import extends WP_Importer {
	...
	// Change for Ohio theme purposes
...
<?php
	...
	function process_posts() {
		...
		foreach ( $this->posts as $post ) {
			...
			if ( $post['post_type'] == 'post' && ! $this->import_options['posts'] ) {
				continue;
			}
			if ( $post['post_type'] == 'product' && ! $this->import_options['products'] ) {
				continue;
			}
			if ( $post['post_type'] == 'ohio_portfolio' && ! $this->import_options['projects'] ) {
				continue;
			}
			if ( $post['post_type'] == 'attachment' && ! $this->import_options['media'] ) {
				continue;
			}
			if ( $post['post_type'] == 'wpcf7_contact_form' && !$this->import_options['forms'] ) {
				continue;
			}
